...
*/
	...
	public  void fyllPaaAttributter(HttpServletRequest request,EAO eao) {
		...
		/**
		 * Pr�ver � formattere input slik at det stemmer.
		 * 
		 * @param fornavn
		 * @return fornavn formattert.
		 */
		try {
		String s1 = fornavn.substring(0, 1).toUpperCase();
	    String fornavnCapitalized = s1 + fornavn.substring(1);
	    fornavn = fornavnCapitalized;
		} catch (Exception e) {
		 ...
		 * 
		 * @param etternavn
		 * @return etternavn formattert.
		 */
		try {
	    String s2 = etternavn.substring(0, 1).toUpperCase();
	    String etternavnCapitalized = s2 + etternavn.substring(1);
	    etternavn = etternavnCapitalized;
		} catch (Exception e){
	...
	public  boolean isValidForNavn(String fornavn) {
		...
		if (fornavn == null) {
			...
			if (! fornavn.matches("[a-zA-z&#230&#248&#229ÆØÅ_ ]+")) {
	...
	public  boolean isValidEtterNavn(String etternavn) {
		...
		if (etternavn == null) {
			...
			if (! etternavn.matches("[a-zA-z&#230&#248&#229ÆØÅ_]+")) {
